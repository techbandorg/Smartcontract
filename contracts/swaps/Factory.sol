...
contract BEP20 is IBEP20 {
    string public constant override name = ' LP';
    string public constant override symbol = 'TB_LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(deadline >= block.timestamp, 'TB: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'TB: INVALID_SIGNATURE');
...
contract Pair is IPair, BEP20 {
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'TB: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external override {
        require(msg.sender == factory, 'TB: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= (2**112 - 1) && balance1 <= (2**112 - 1), 'TB: OVERFLOW'); // uint112(-1) = 2 ** 112 - 1
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'TB: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external override lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'TB: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
        require(amount0Out > 0 || amount1Out > 0, 'TB: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'TB: INSUFFICIENT_LIQUIDITY');
        ...
        require(balance0Adjusted * balance1Adjusted >= uint(_reserve0) * _reserve1 * (10000**2), ': K');
        }
...
contract Factory is IFactory {
    ...
    // address public override ReferralProgram;

    mapping(address => mapping(address => address)) public override getPair;
    address[] public override allPairs;

    constructor(address _feeToSetter) {
        require(_feeToSetter != address(0), "TB: Zero address");
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'TB: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'TB: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'TB: PAIR_EXISTS'); // single check is sufficient
