...
contract LPReward is Ownable {
    ...
    event RecordRemoveLiquidityGiveTbt(address indexed recipient, address indexed pair, uint Tbt, uint amountA, uint amountB, uint liquidity);
    event ClaimLiquidityTbt(address indexed recipient, address indexed pair, uint Tbt, uint amountA, uint amountB);
    event Rescue(address indexed to, uint amount);
    event RescueToken(address indexed token, address indexed to, uint amount); 

    constructor(address tbt, address factory) {
        require(tbt != address(0) && factory != address(0), "LPReward: Zero address(es)");
    ...
    function recordRemoveLiquidity(address recipient, address tokenA, address tokenB, uint amountA, uint amountB, uint liquidity) external lock onlyRouter { 
        ...
        uint amountTbt;
        if (tokenA != TBT && tokenB != TBT) {
            address tokenToTbtPair = swapFactory.getPair(tokenA, TBT);
            if (tokenToTbtPair != address(0)) {
                amountTbt = IRouter(swapRouter).getAmountsOut(amount0, getPathForToken(tokenA))[1];
            }

            tokenToTbtPair = swapFactory.getPair(tokenB, TBT);
            if (tokenToTbtPair != address(0)) {
                if (amountTbt != 0) {
                    amountTbt = amountTbt + IRouter(swapRouter).getAmountsOut(amount1, getPathForToken(tokenB))[1];
                } else  {
                    amountTbt = IRouter(swapRouter).getAmountsOut(amount1, getPathForToken(tokenB))[1] * 2;
                }
            } else {
                amountTbt = amountTbt * 2;
            }
        } else if (tokenA == TBT) { 
            amountTbt = amount0 * 2;
        } else {
        ...
        if (amountTbt != 0 && amountTbt <= availableReward() && IBEP20(TBT).balanceOf(address(this)) >= amountTbt) {
            require(IBEP20(TBT).transfer(recipient, amountTbt), "LPReward: Erroe while transfering");
            lpRewardUsed = lpRewardUsed + amountTbt;
            emit RecordRemoveLiquidityGiveTbt(recipient, pair, amountTbt, amountA, amountB, liquidity);            
        } else {
            uint amountS0;
            uint amountS1;
            {
    ...
    function claimBonus(address pair, address recipient) public lock {
        ...
        uint amountTbt = tbtAmountForPair(pair, amountA, amountB);
        require (amountTbt > 0, "LPReward: No Tbt pairs to token A and token B");
        require (amountTbt <= availableReward(), "LPReward: Available reward for the period is used");
        
        require(IBEP20(TBT).transfer(recipient, amountTbt), "LPReward: Error while transfering");
        lpRewardUsed += amountTbt;
        emit ClaimLiquidityTbt(recipient, pair, amountTbt, amountA, amountB);            
    }

    function unclaimedAmountTbt(address recipient, address pair) external view returns (uint) {
        uint amountA;
        uint amountB;
        if (unclaimedAmounts[recipient][pair].length != 0) {
            amountA = unclaimedAmounts[recipient][pair][0];
        ...
        return tbtAmountForPair(pair, amountA, amountB);
    }
    ...
    function tbtAmountForPair(address pair, uint amountA, uint amountB) private view returns (uint amountTbt) {
        address tokenA = pairTokens[pair][0];
        address tokenB = pairTokens[pair][1];
        if (tokenA != TBT && tokenB != TBT) {
            address tokenToTbtPair = swapFactory.getPair(tokenA, TBT);
            if (tokenToTbtPair != address(0)) {
                amountTbt = IRouter(swapRouter).getAmountsOut(amountA, getPathForToken(tokenA))[1];
            }

            tokenToTbtPair = swapFactory.getPair(tokenB, TBT);
            if (tokenToTbtPair != address(0)) {
                if (amountTbt != 0) {
                    amountTbt = amountTbt + IRouter(swapRouter).getAmountsOut(amountB, getPathForToken(tokenB))[1];
                } else  {
                    amountTbt = IRouter(swapRouter).getAmountsOut(amountB, getPathForToken(tokenB))[1] * 2;
                }
            } else {
                amountTbt = amountTbt * 2;
            }
