...
contract LockStakingFixedAPY is ILockStakingRewards, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        require(_token != address(0), "LockStakingFixedAPY: Zero address");
        token = IBEP20(_token);
    ...
    function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant {
        require(amount > 0, "LockStakingFixedAPY: Cannot stake 0");
        _totalSupply += amount;
    ...
    function stakeFor(uint256 amount, address user) external override nonReentrant {
        require(amount > 0, "LockStakingFixedAPY: Cannot stake 0");
        require(user != address(0), "LockStakingFixedAPY: Cannot stake for zero address");
        _totalSupply += amount;
    ...
    function withdraw(uint256 nonce) public override nonReentrant {
        ...
        require(stakeAmounts[msg.sender][nonce] > 0, "LockStakingFixedAPY: This stake nonce was withdrawn");
        require(stakeLocks[msg.sender][nonce] < block.timestamp, "LockStakingFixedAPY: Locked");
        _totalSupply -= amount;
    ...
    function rescue(address to, address tokenAddress, uint256 amount) external onlyOwner {
        require(to != address(0), "LockStakingFixedAPY: Cannot rescue to the zero address");
        require(amount > 0, "LockStakingFixedAPY: Cannot rescue 0");
        require(tokenAddress != address(token), "LockStakingFixedAPY: Cannot rescue staking/reward token");
