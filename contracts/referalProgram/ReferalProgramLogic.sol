...
contract ReferralProgramLogic is Ownable { 
    IReferralProgram public immutable users;
    IBEP20 public immutable TBT;
    ...
    constructor(address referralUsers, address tbt)  {
        require(referralUsers != address(0) && tbt != address(0), "Referral: Address is zero");
        levels = [40, 20, 13, 10, 10, 7];
        maxLevel = 6;
        TBT = IBEP20(tbt);
        users = IReferralProgram(referralUsers);
    ...
    function isUserBalanceEnough(address user) public view returns (bool) {
        ...
        uint amount = TBT.balanceOf(user);
    ...
    function claimSpecialReserveFund(address token) public {
        ...
        require(newSwapRouter != address(0), "Referral: Address is zero");
        swapRouter = IRouter(newSwapRouter);
    }

    function updateSwapToken(address newSwapToken) external onlyOwner {
        require(newSwapToken != address(0), "Referral: Address is zero");
    ...
    function updateStakingPoolAdd(address newStakingPool) external onlyOwner {
        IStakingPool pool = IStakingPool(newStakingPool);
        require (pool.stakingToken() == TBT, "Referral: Wrong pool staking tokens");

        for (uint i; i < stakingPools.length; i++) {
            require (address(stakingPools[i]) != newStakingPool, "Referral: Pool exists");
        }
        stakingPools.push(IStakingPool(pool));
    }
    ...
    function updateLevels(uint[] memory newLevels) external onlyOwner {
        ...
        require(checkSum == 100, "Referral: Wrong levels amounts");
        levels = newLevels;
