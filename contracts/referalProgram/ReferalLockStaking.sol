...
interface IReferralProgramUsers{
...
contract ReferralLockStaking is ILockStakingRewards, ReentrancyGuard, Ownable {
    ...
    IReferralProgramUsers public referralProgramUsers;
    ...
    function stakeWithPermit(uint256 amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external nonReentrant {
        require(amount > 0, "ReferralLockStaking: Cannot stake 0");

        if(onlyAllowedAddresses) {
            require(allowedAddresses[msg.sender], "ReferralLockStaking: Only allowed addresses.");
        }
    ...
    function stakeFor(uint256 amount, address user) external override nonReentrant {
        require(amount > 0, "ReferralLockStaking: Cannot stake 0");
        require(user != address(0), "ReferralLockStaking: Cannot stake for zero address");

        if(onlyAllowedAddresses) {
            require(allowedAddresses[user], "ReferralLockStaking: Only allowed addresses.");
        }
    ...
    function withdraw(uint256 nonce) public override nonReentrant {
        ...
        require(stakeInfo[msg.sender][nonce].stakeAmount > 0, "ReferralLockStaking: This stake nonce was withdrawn");
        require(stakeInfo[msg.sender][nonce].stakeLock < block.timestamp, "ReferralLockStaking: Locked");
        _totalSupply -= amount;
    ...
    function getUserReferralId(address account) external view returns (uint256) {
        require(address(referralProgramUsers) != address(0), "ReferralLockStaking: Referral Program was not added.");
    ...
    function rescue(address to, address tokenAddress, uint256 amount) external onlyOwner {
        require(to != address(0), "ReferralLockStaking: Cannot rescue to the zero address");
        require(amount > 0, "ReferralLockStaking: Cannot rescue 0");
        require(tokenAddress != address(token), "ReferralLockStaking: Cannot rescue staking/reward token");
    ...
    function updateAllowedAddress(address _address, bool allowance) public onlyOwner {
        require(_address != address(0), "ReferralLockStaking: allowed address can't be equal to address(0)");
    ...
    function updateAllowedAddresses(address[] memory addresses, bool[] memory allowances) external onlyOwner {
        require(addresses.length == allowances.length, "ReferralLockStaking: Addresses and allowances arrays have different size.");
    ...
    function updateRewardRate(uint256 _rewardRate) external onlyOwner {
        require(_rewardRate > 0, "ReferralLockStaking: Reward rate must be grater than 0");
        rewardRate = _rewardRate;
        emit RewardRateUpdated(rewardRate);
    }
    
    function updateReferralRewardRate(uint256 _referralRewardRate) external onlyOwner {
        require(_referralRewardRate >= rewardRate, "ReferralLockStaking: Referral reward rate can't be lower than reward rate");
        referralRewardRate = _referralRewardRate;
