...
contract ReferralProgramUsers is IReferralProgram, Ownable {
    ...
    constructor(address migratorAddress)  {
        require(migratorAddress != address(0), " Referral: Zero address");
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("ReferralProgram")),
                keccak256(bytes('1')),
                chainId,
                address(this)
            )
    ...
    modifier onlyMigrator() {
        require(msg.sender == migrator, " Referral: Caller is not the migrator");
        _;
    }

    modifier onlyRegistrator() {
        require(registrators[msg.sender], " Referral: Caller is not the registrator");
        _;
    }
    ...
    function _registerUser(address user, uint sponsorId, uint category) private returns (uint) {
        require(user != address(0), " Referral: Address is zero");
        require(userIdByAddress[user] == 0, " Referral: Already registered");
        // require(_userSponsor[sponsorId] != 0, " Referral: No such sponsor");
    ...
    function migrateUsers(uint[] memory ids, uint[] memory sponsorId, address[] memory userAddress) external onlyMigrator {
        require(lastUserId == 0, " Referral: Basic migration is finished");
        require(ids.length == sponsorId.length, " Referral: Different array lengths");     
    ...
    function updateUserAddress(uint id, address userAddress) external onlyMigrator {
        require(userAddress != address(0), " Referral: Address is zero");
        require(_userSponsor[id] > 1000000000, " Referral: No such user");
        require(userIdByAddress[userAddress] == 0, " Referral: Address is already in the system");
    ...
    function updateUserAddressBySig(uint id, address userAddress, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(block.timestamp <= deadline, " Referral: Signature expired");
        require(userIdByAddress[userAddress] == 0, " Referral: Address is already in the system");
        ...
        require(recoveredAddress != address(0) && recoveredAddress == migrator, ' Referral: Invalid signature');
        userIdByAddress[userAddress] = id;
        userAddressById[id] = userAddress;
    ...
    function updateMigrator(address newMigrator) external {
        require(msg.sender == migrator || msg.sender == owner, " Referral: Not allowed");
        require(newMigrator != address(0), " Referral: Address is zero");
